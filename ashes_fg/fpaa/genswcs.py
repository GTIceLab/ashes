...
# dirx = str(Path(__file__).resolve().parent) + '/'
...
def main(
    bs_circuit_loc, bs_run_dir, bs_board_arch, bs_vpr_disp=None, bs_pins_file=None
):
    ...
    if bs_circuit_loc != None:
        circuit_loc = bs_circuit_loc
    if bs_run_dir != None:
        run_dir = bs_run_dir
    if bs_vpr_disp != None:
    ...
    fixed_locations = extract_fixed_locations(
        circuit_loc + ".blif"
    )  # fixed_locations stores [subblock name, x location, y location]
    ...
    if not arch_file:
        arch_file = dirx + parseBlif(circuit_loc + ".blif")
    xarray = pbarray(
        len(chipStats().array.pattern), len(chipStats().array.pattern[0])
    )  # initial import

    ## Call first pass VTR
    print(
        f"{RASPPATH}/vtr_release/vpr/vpr {ASHESPATH}/ashes_fg/fpaa/arch/"
        + bs_board_arch
        + "_arch.xml "
        + bs_circuit_loc
        + "  -route_chan_width 17 -timing_analysis off -fix_pins "
        + bs_circuit_loc
        + ".pads -nodisp"
    )
    os.system(
    ...
    fixed_locations = extract_net_values(
        "%s/%s.net" % (run_dir, circuit_name), fixed_locations
    )
    print(fixed_locations)
    ...
    if pins_file:
        parsePads(pins_file)
    parseRoute(
        "%s/%s.route" % (run_dir, circuit_name)
    )  # also generates global interconnect switch list
    genLISwcs()
...
def runVTR(arch_file, circuit, pins_file, run_dir, vpr_disp):
    DEBUG = 0
    vpr_loc = f"{RASPPATH}/vtr_release/vpr/vpr"
    chan_width = 17
    ...
    exec_str = "%s %s -route %s " % (vpr_loc, arch_file, circuit)
    ...
    """
    if chan_width > 0:
        exec_str = exec_str + " -route_chan_width %g" % (chan_width)
    if pins_file and fix_cab == 0:
        exec_str = exec_str + " -fix_pins %s" % (pins_file)
    if vpr_disp == 0:
        exec_str = exec_str + " -nodisp"
    if ff_custom > 0:
        exec_str = exec_str + " -timing_analysis off"  # Michelle added on
...
def genLocalInterconnectSwcs():
    ...
    for block_name in pblocks.keys():
        ...
        print(
            "~~~~~ LI for BLOCK: %s %s (%s %s) ~~~~~~"
            % (block_name, tile_type, grid_loc[0], grid_loc[1])
        )
        if tile_type[:2] == "io":
...
def printSwcs(file_name):
    ...
    for i in range(-1, len(swcs) - 1):
        if swcs[i + 1][1] in list(range(0x281, 0x28F)) + list(
            range(0x291, 0x29F)
        ) + list(range(0x2A1, 0x2AF)) + list(range(0x2B1, 0x2BF)) + list(
            range(0x2C1, 0x2CF)
        ) + list(
            range(0x2D1, 0x2DF)
        ) + list(
        ...
        f.write(
            "%s %s %s %s"
            % (swcs[i + 1][0], swcs[i + 1][1], swcs[i + 1][2], swcs[i + 1][3])
            + "\n"
        )
...
def getSwcSswitch(direction, track, grid_loc, net_name):
    ...
    if VERBOSE == 1 or VERBOSE > 1:
        print(
            ...
            % (
                tile_type,
                str(tile_loc[0]).rjust(2),
                str(tile_loc[1]).rjust(2),
                direction,
                str(swc[0]),
                str(swc[1]),
            )
...
# &&&&&&&&&&&&&&&&&
def getSwcCblock(
    pin_num, pin_grid_loc, cblock_type, track, cblock_grid_loc, net_name, c_outin
):
    if cap_nams[-1] != net_name:
        cap_nams.append(net_name)
    ...
    pin_tile_type = getTileType(
        "CHANX", pin_grid_loc
    )  # where the pin (source or sink) is WAS HARDCODED TO CHANX before::
    ...
    if rasp3a == 0:
        ...
        # A-A and D-D case
            ...
            cblock_grid_loc[0] in [3, 5, 7, 9, 11]
            and cblock_tile_loc != pin_tile_loc
            and (pin_tile_loc[0] - cblock_tile_loc[0] != 1)
            and cblock_type == "CHANY"
        ):
            if pin_name[0] != "X":
                print("lala-land")
    ...
    if VERBOSE:
        print(
            "%s [%s,%s] %s  (%s,%s) %s %s"
            % (
                cblock_tile_type.rjust(4),
                str(cblock_tile_loc[0]).rjust(2),
                str(cblock_tile_loc[1]).rjust(2),
                cblock_type,
                str(swc[0]),
                str(swc[1]),
                track_name,
    ...
    if VERBOSE > 1:
        ...
        print(
            cblock_type
            + " in "
            + str(cblock_grid_loc[0])
            + " "
            + str(cblock_grid_loc[1])
        )
...
def getBlockName(grid_loc):
    for block_name in pblocks.keys():
        block = pblocks[block_name]
        if block["grid_loc"] == grid_loc or block["grid_loc"] == [
            str(grid_loc[0]),
            str(grid_loc[1]),
        ]:
            return block_name
...
def getTileLoc(block_type, grid_loc):
    ...
    if block_type in [
        "CHANY",
        "SBLOCK",
    ]:  # and tile_loc[0] != 0: ## removed CHANY and SBLOCK as option
            ...
            for subblock in subblocks:
                ...
                if (sub_inst[:-3] == "counter8") & (sub_outputs != "open"):
                    ...
                    for blenum in range(8):
                        sub_output = (
                            subblock.findall("outputs")[0].findall("port")[0].text
                        )
                    ...
                    sub_input_str = (
                        subblock.findall("inputs")[0].findall("port")[0].text
                    )
                    ...
                    if sub_inst[:3] == "ble":
                        ...
                        for sub3blk in sub3blks:
                            ...
                            if (
                                sub3_inst[:-3] == "soft_logic"
                            ):  ##can't use the latch standalone
                                cover = luts[
                                    sub3_outputs
                                ].cover  # index luts by name == output
                                nsb = pblock(sub_inst, "lut")
                                nsb.ex_fgs = cover  # LUT cover comes from blif parsing
                                nsb.inputs_orig = luts[sub3_outputs].inputs
                        mux_use = (
                            subblock.findall("outputs")[0].findall("port")[0].text[0:2]
                        )
                        if mux_use == "ff":
            ...
            if counter_flag == 0:
                for i in range(len(nb.subblocks)):
                    ...
                    for i in range(len(sub_inputs)):
                        ...
                        if (
                            cur_input.split(".")[0] == block_type
                        ):  # input from complex block input
                            in_num = int(
                                cur_input.split("[")[-1].split("]")[0]
                            )  # cab.I[13] -> 13
                            sub_input_names.append(nb.inputs[in_num])
                        elif cur_input == "open":
                            sub_input_names.append("open")
                        else:
                            sub_input_names.append(
                                nb.getSub(cur_input.split(".")[0]).outputs
                            )
                ...
                for i in range(len(outputs)):
                    if outputs[i] != "open":
                        from_sub_name = outputs[i].split(".")[
                            0
                        ]  # ota[0].out[0] -> ota[0]
                        if from_sub_name == ["SR4[0]"]:
                            try:
                                if i < 4:
                                    output_net_names[i] = nb.getSub(
                                        from_sub_name
                                    ).outputs[i]
                                else:
                                    output_net_names[i] = nb.getSub(
        ...
        if go:
            ...
            if new_block.type[:7] == "ioslice":
                ...
                if new_block.type[8:] == "inpad":
                    xarray.getSub(grid_loc).setPort(
                        sub_num * 3 + 1, block_name
                    )  # setting output port
                else:
                    xarray.getSub(grid_loc).setPort(
                        sub_num * 3 + 0, block_name
                    )  # setting input port
...
def parseBlif(file_name):
        ...
        if len(words):
            if words[0] == ".subckt":
                ...
                if len(line.split("#")) > 1:
                    ex_fgs = line.split("#")[
                        1
                    ].rstrip()  # parameters after # in .blif file.
                else:
                    ex_fgs = 0
                ...
                if subckt in [
                    ...
                    key = ports[
                        1
                    ]  # this key is just output port name ## for all cases with a single i/p- o/p
                elif subckt in [
                    "DAC_sftreg",
                    "latch_custom",
                    "sftreg2",
    ...
    arch_file = f"{ASHESPATH}/ashes_fg/fpaa/arch/rasp3_arch.xml"
    ...
    for line in f:
        ...
        if len(cur):
            if cur[0] == "Net" and cur[2] != "(gnd):":
                ...
                if last:
                    cgl = [
                        int(i) for i in cur[1][1:-1].split(",")
                    ]  # current grid location
                    lgl = [
                        int(i) for i in last[1][1:-1].split(",")
                    ]  # last grid location
                    if lastlast != []:
                        if (
                            lastlast[0] == "OPIN"
                    ...
                    if last[0][:-1] == "CHAN" and cur[0][:-1] == "CHAN":
                        ...
                        if last[0] == "CHANY":
                            if cur[0] == "CHANY":
                                if block_dir == [0, 1]:
                                    swc = getSwcSswitch(
                                        "sn", track, lgl, new_route["name"]
                                    )
                                else:
                                    swc = getSwcSswitch(
                                        "ns", track, cgl, new_route["name"]
                                    )
                            elif cur[0] == "CHANX":
                                if block_dir == [0, 0]:
                                    swc = getSwcSswitch(
                                        "sw", track, lgl, new_route["name"]
                                    )
                                elif block_dir == [1, 0]:
                                    swc = getSwcSswitch(
                                        "se", track, lgl, new_route["name"]
                                    )
                                elif block_dir == [0, -1]:
                                    swc = getSwcSswitch(
                                        "nw", track, cgl, new_route["name"]
                                    )
                                elif block_dir == [1, -1]:
                                    swc = getSwcSswitch(
                                        "ne",
                            ...
                            if cur[0] == "CHANX":
                                if block_dir == [1, 0]:
                                    swc = getSwcSswitch(
                                        "we", track, lgl, new_route["name"]
                                    )
                                else:
                                    swc = getSwcSswitch(
                                        "ew", track, cgl, new_route["name"]
                                    )
                            if cur[0] == "CHANY":
                                if block_dir == [0, 1]:
                                    swc = getSwcSswitch(
                                        "wn", track, lgl, new_route["name"]
                                    )
                                elif block_dir == [0, 0]:
                                    swc = getSwcSswitch(
                                        "ws", track, lgl, new_route["name"]
                                    )  # can be lgl or cgl
                                elif block_dir == [-1, 0]:
                                    swc = getSwcSswitch(
                                        "es", track, cgl, new_route["name"]
                                    )
                                elif block_dir == [-1, 1]:
                                    swc = getSwcSswitch(
                                        "en",
                                    ...
                                    0,
                                    -1,
                                ]:  ##special case when branching...so create CHANY->CHANY case
                                    swc = getSwcSswitch(
                                        "sn", track, cgl, new_route["name"]
                                    )
                                else:
                                    print("SBLOCK switch error")
                                    print(block_dir)
                        ...
                        print(
                            "~~~~ PIN SINK ~~~~ || net %s -> pin %s in %s %s"
                            % (new_route["name"], cur[3], cgl[0], cgl[1])
                        )

                        # update pin order
                        xarray.getSub(int(cgl[0]), int(cgl[1])).movePort(
                            new_route["name"][1:-1], tile_t
                        )
