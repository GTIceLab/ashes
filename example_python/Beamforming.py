...
import ashes_fg.class_lib_beamform as lib_beam
...
#Delaylines = lib_beam.TOP_DelayLines(Top,DelaylinesIsland,[1,1])
Delaylines.place([0,0])

#ALICE right
VMMWTAIsland = Island(Top)
VMMWTA = lib_beam.VMMWTAAlgFullRoute(Top,VMMWTAIsland,[1,1])
VMMWTA.place([0,0])

ModulationIsland = Island(Top)
Modulation = lib_beam.ModulationAlgFlip(Top,ModulationIsland,[1,1])
Modulation.place([0,0])

# Delaylines.OUTS += Modulation.RUNO
Delaylines.n_OUTS[0] += Modulation.RUNO[0]
